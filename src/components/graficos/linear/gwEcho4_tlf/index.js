...
//gwEcho4_tlf
class GwEcho4_tlf extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("https://headspace-back.azurewebsites.net/headspace_dev1").then(res => {
        ...
        let gwEcho4_tlf_array = [];

       novo_array.map(function(i){
        gwEcho4_tlf_array.push({
                "gwEcho4_tlf": i.gwEcho4_tlf,
            })
            return gwEcho4_tlf_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho4_tlf_array_1 = gwEcho4_tlf_array.map(a => a.gwEcho4_tlf);  
        let gwEcho4_tlf_array_2=[];

        j =0;

        for (i=0; i<gwEcho4_tlf_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho4_tlf_array_2[j]=gwEcho4_tlf_array_1[i];
                    j = j+1;
            } else {
            }
        }
