...
//gwEcho2_temp
class GwEcho2_temp extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("https://headspace-back.azurewebsites.net/headspace_dev1").then(res => {
        ...
        let gwEcho2_temp_array = [];

       novo_array.map(function(i){
        gwEcho2_temp_array.push({
                "gwEcho2_temp": i.gwEcho2_temp,
            })
            return gwEcho2_temp_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho2_temp_array_1 = gwEcho2_temp_array.map(a => a.gwEcho2_temp);  
        let gwEcho2_temp_array_2=[];

        j =0;

        for (i=0; i<gwEcho2_temp_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho2_temp_array_2[j]=gwEcho2_temp_array_1[i];
                    j = j+1;
            } else {
            }
        }
