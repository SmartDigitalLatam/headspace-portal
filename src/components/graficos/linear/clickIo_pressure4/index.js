...
class ClickIo_pressure4 extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("http://localhost:3000/headspace_dev1").then(res => {
        ...
        let ClickIo_pressure4_array = [];

       novo_array.map(function(i){
        ClickIo_pressure4_array.push({
                "ClickIo_pressure4": i.ClickIo_pressure4,
            })
            return ClickIo_pressure4_array;
        })
       ...
       //manipulando o array de dados
        let ClickIo_pressure4_array_1 = ClickIo_pressure4_array.map(a => a.ClickIo_pressure4);  
        let ClickIo_pressure4_array_2=[];

        j =0;

        for (i=0; i<ClickIo_pressure4_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                ClickIo_pressure4_array_2[j]=ClickIo_pressure4_array_1[i];
                    j = j+1;
            } else {
            }
        }
        ...
        //plotagem dos arrays limpos
            ...
            this.setState({
                chartData:{
                    ...
                    datasets: [
                        {
                            label: "ClickIo_pressure4",
                            data: ClickIo_pressure4_array_2,
                            lineTension: 0.1,
                            fill:false,
    ...
    render(){
        return(
            <div className = "ClickIo_pressure4">
...
export default ClickIo_pressure4;