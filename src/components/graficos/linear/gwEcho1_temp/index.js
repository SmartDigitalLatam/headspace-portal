...
//gwEcho1_temp
class GwEcho1_temp extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("https://headspace-back.azurewebsites.net/headspace_dev1").then(res => {
        ...
        let gwEcho1_temp_array = [];

       novo_array.map(function(i){
        gwEcho1_temp_array.push({
                "gwEcho1_temp": i.gwEcho1_temp,
            })
            return gwEcho1_temp_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho1_temp_array_1 = gwEcho1_temp_array.map(a => a.gwEcho1_temp);  
        let gwEcho1_temp_array_2=[];

        j =0;

        for (i=0; i<gwEcho1_temp_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho1_temp_array_2[j]=gwEcho1_temp_array_1[i];
                    j = j+1;
            } else {
            }
        }
