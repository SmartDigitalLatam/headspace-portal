...
//gwEcho4_tlf_carry
class GwEcho4_tlf_carry extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("http://localhost:3000/headspace_dev1").then(res => {
        ...
        let gwEcho4_tlf_carry_array = [];

       novo_array.map(function(i){
        gwEcho4_tlf_carry_array.push({
                "gwEcho4_tlf_carry": i.gwEcho4_tlf_carry,
            })
            return gwEcho4_tlf_carry_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho4_tlf_carry_array_1 = gwEcho4_tlf_carry_array.map(a => a.gwEcho4_tlf_carry);  
        let gwEcho4_tlf_carry_array_2=[];

        j =0;

        for (i=0; i<gwEcho4_tlf_carry_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho4_tlf_carry_array_2[j]=gwEcho4_tlf_carry_array_1[i];
                    j = j+1;
            } else {
            }
        }
        ...
        //plotagem dos arrays limpos
            ...
            this.setState({
                chartData:{
                    ...
                    datasets: [
                        {
                            label: "gwEcho4_tlf_carry",
                            data: gwEcho4_tlf_carry_array_2,
                            lineTension: 0.1,
                            fill:false,
    ...
    render(){
        return(
            <div className = "gwEcho4_tlf_carry">
...
export default GwEcho4_tlf_carry;