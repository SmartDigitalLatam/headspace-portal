...
//gwEcho2_tlf_carry
class GwEcho2_tlf_carry extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("https://headspace-back.azurewebsites.net/headspace_dev1").then(res => {
        ...
        let gwEcho2_tlf_carry_array = [];

       novo_array.map(function(i){
        gwEcho2_tlf_carry_array.push({
                "gwEcho2_tlf_carry": i.gwEcho2_tlf_carry,
            })
            return gwEcho2_tlf_carry_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho2_tlf_carry_array_1 = gwEcho2_tlf_carry_array.map(a => a.gwEcho2_tlf_carry);  
        let gwEcho2_tlf_carry_array_2=[];

        j =0;

        for (i=0; i<gwEcho2_tlf_carry_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho2_tlf_carry_array_2[j]=gwEcho2_tlf_carry_array_1[i];
                    j = j+1;
            } else {
            }
        }
        ...
        //plotagem dos arrays limpos
            ...
            this.setState({
                chartData:{
                    ...
                    datasets: [
                        {
                            label: "gwEcho2_tlf_carry",
                            data: gwEcho2_tlf_carry_array_2,
                            lineTension: 0.1,
                            fill:false,
    ...
    render(){
        return(
            <div className = "gwEcho2_tlf_carry">
...
export default GwEcho2_tlf_carry;