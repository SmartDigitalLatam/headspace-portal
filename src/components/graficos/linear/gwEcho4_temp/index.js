...
//gwEcho4_temp
class GwEcho4_temp extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("https://headspace-back.azurewebsites.net/headspace_dev1").then(res => {
        ...
        let gwEcho4_temp_array = [];

       novo_array.map(function(i){
        gwEcho4_temp_array.push({
                "gwEcho4_temp": i.gwEcho4_temp,
            })
            return gwEcho4_temp_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho4_temp_array_1 = gwEcho4_temp_array.map(a => a.gwEcho4_temp);  
        let gwEcho4_temp_array_2=[];

        j =0;

        for (i=0; i<gwEcho4_temp_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho4_temp_array_2[j]=gwEcho4_temp_array_1[i];
                    j = j+1;
            } else {
            }
        }
