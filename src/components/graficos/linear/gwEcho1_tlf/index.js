...
//gwEcho1_tlf
class GwEcho1_tlf extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("https://headspace-back.azurewebsites.net/headspace_dev1").then(res => {
        ...
        let gwEcho1_tlf_array = [];

       novo_array.map(function(i){
        gwEcho1_tlf_array.push({
                "gwEcho1_tlf": i.gwEcho1_tlf,
            })
            return gwEcho1_tlf_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho1_tlf_array_1 = gwEcho1_tlf_array.map(a => a.gwEcho1_tlf);  
        let gwEcho1_tlf_array_2=[];

        j =0;

        for (i=0; i<gwEcho1_tlf_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho1_tlf_array_2[j]=gwEcho1_tlf_array_1[i];
                    j = j+1;
            } else {
            }
        }
