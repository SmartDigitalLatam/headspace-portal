...
//gwEcho4_flow_carry
class GwEcho4_flow_carry extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("http://localhost:3000/headspace_dev1").then(res => {
        ...
        let gwEcho4_flow_carry_array = [];

       novo_array.map(function(i){
        gwEcho4_flow_carry_array.push({
                "gwEcho4_flow_carry": i.gwEcho4_flow_carry,
            })
            return gwEcho4_flow_carry_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho4_flow_carry_array_1 = gwEcho4_flow_carry_array.map(a => a.gwEcho4_flow_carry);  
        let gwEcho4_flow_carry_array_2=[];

        j =0;

        for (i=0; i<gwEcho4_flow_carry_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho4_flow_carry_array_2[j]=gwEcho4_flow_carry_array_1[i];
                    j = j+1;
            } else {
            }
        }
        ...
        //plotagem dos arrays limpos
            ...
            this.setState({
                chartData:{
                    ...
                    datasets: [
                        {
                            label: "gwEcho4_flow_carry",
                            data: gwEcho4_flow_carry_array_2,
                            lineTension: 0.1,
                            fill:false,
    ...
    render(){
        return(
            <div className = "gwEcho4_flow_carry">
...
export default GwEcho4_flow_carry;