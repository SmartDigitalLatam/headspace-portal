...
//gwEcho2_tlf
class GwEcho2_tlf extends Component{
    constructor(props){
        super(props);
        this.state = {
            chartData:{}
    ...
    getChartData(){
       ...
       axios.get("http://localhost:3000/headspace_dev1").then(res => {
        ...
        let gwEcho2_tlf_array = [];

       novo_array.map(function(i){
        gwEcho2_tlf_array.push({
                "gwEcho2_tlf": i.gwEcho2_tlf,
            })
            return gwEcho2_tlf_array;
        })
       ...
       //manipulando o array de dados
        let gwEcho2_tlf_array_1 = gwEcho2_tlf_array.map(a => a.gwEcho2_tlf);  
        let gwEcho2_tlf_array_2=[];

        j =0;

        for (i=0; i<gwEcho2_tlf_array_1.length;i++){
            if (property_array_1[i]==="telemetry"){
                gwEcho2_tlf_array_2[j]=gwEcho2_tlf_array_1[i];
                    j = j+1;
            } else {
            }
        }
